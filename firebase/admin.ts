import { initializeApp, getApps, cert, AppOptions } from "firebase-admin/app";
import { getAuth } from "firebase-admin/auth";
import { getFirestore } from "firebase-admin/firestore";

// Hardcoded Firebase config as fallback
const FALLBACK_PROJECT_ID = "wapi-40b90";
const FALLBACK_CLIENT_EMAIL = "firebase-adminsdk-fbsvc@wapi-40b90.iam.gserviceaccount.com";
const FALLBACK_PRIVATE_KEY = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n";

// Initialize Firebase Admin SDK
function initFirebaseAdmin() {
  const apps = getApps();

  if (!apps.length) {
    try {
      let appOptions: AppOptions = {};
      
      // Try environment variables first
      if (process.env.FIREBASE_PROJECT_ID && 
          process.env.FIREBASE_CLIENT_EMAIL && 
          process.env.FIREBASE_PRIVATE_KEY) {
        console.log("Initializing Firebase Admin with environment variables");
        appOptions = {
          credential: cert({
            projectId: process.env.FIREBASE_PROJECT_ID,
            clientEmail: process.env.FIREBASE_CLIENT_EMAIL,
            privateKey: process.env.FIREBASE_PRIVATE_KEY.replace(/\\n/g, "\n"),
          }),
        };
      } 
      // Then try local service account file
      else {
        try {
          console.log("Trying to initialize Firebase Admin with local service account");
          const serviceAccount = require("../wapi-40b90-firebase-adminsdk-fbsvc-38b6e18f63.json");
          appOptions = {
            credential: cert(serviceAccount),
          };
        } catch (fileError) {
          // Finally, use hardcoded credentials as last resort
          console.log("Using hardcoded Firebase credentials as fallback");
          appOptions = {
            credential: cert({
              projectId: FALLBACK_PROJECT_ID,
              clientEmail: FALLBACK_CLIENT_EMAIL,
              privateKey: FALLBACK_PRIVATE_KEY,
            }),
          };
        }
      }
      
      // Initialize the app with the best available credentials
      initializeApp(appOptions);
    } catch (error) {
      console.error("Failed to initialize Firebase Admin with credentials:", error);
      
      // Last resort: Initialize with minimal config
      try {
        console.log("Initializing Firebase Admin with minimal config");
        initializeApp({
          projectId: FALLBACK_PROJECT_ID
        });
      } catch (finalError) {
        console.error("Could not initialize Firebase Admin at all:", finalError);
      }
    }
  }

  return {
    auth: getAuth(),
    db: getFirestore(),
  };
}

export const { auth, db } = initFirebaseAdmin();
